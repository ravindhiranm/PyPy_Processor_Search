...
def evaluate(total_cores, core, area_constraint, power_constraint):
    ...
    evaluate_range = range(0,total_cores)

    ##temp_list=range(0,total_cores)
    ##t= itertools.permutations(templist,total_cores)
    ...
    for p in itertools.combinations(benchmark_list,total_cores):
        ...
        for r in itertools.permutations(p,total_cores):
            ...
            for i in evaluate_range:
                bips_permutation_total_value+=core[i].get_bips2w(r[i])
...
elif (len(allowed_proc_configs) = 0):
	print "No valid processor configurations found under given constraints. Loosen constraints and try again :) \n"
else:
