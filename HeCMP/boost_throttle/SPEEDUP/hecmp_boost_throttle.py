...
##### Search to do variable voltage-frequency exploration ####
...
def evaluate(total_cores, core, area_constraint, power_constraint):
    ...
    for p in workloads:
        ...
        for j in permute:
            ...
            core0_power_list = core[0].get_app_peak_power(wl1)
            core1_power_list = core[1].get_app_peak_power(wl2)
            core2_power_list = core[2].get_app_peak_power(wl3)
            core3_power_list = core[3].get_app_peak_power(wl4)

            core0_goodness_list = core[0].get_speedup(wl1)
            core1_goodness_list = core[1].get_speedup(wl2)
            core2_goodness_list = core[2].get_speedup(wl3)
            core3_goodness_list = core[3].get_speedup(wl4)

            for f in frequency_settings:
                goodness_value_each_permutation = 0.0
                proc_power=0.0

                proc_power = core0_power_list[f[0]] + core1_power_list[f[1]] + core2_power_list[f[2]] + core3_power_list[f[3]]
                if (proc_power > power_constraint):### Drop schedule if greater than power budget
                    permutations_goodness_container.append(0)
                else:
                    goodness_value_each_permutation += core0_goodness_list[f[0]] + core1_goodness_list[f[1]] + core2_goodness_list[f[2]] + core3_goodness_list[f[3]]
                    ##print goodness_value_each_permutation
...
for index in benchmark_list:
    for core in all_configs:
        speedup=[]
        for freq in range(0,5):
            speedup.append(execution_time(all_configs[0].get_cycles(index)[0],all_configs[0].get_frequency()[0])/execution_time(core.get_cycles(index)[freq],core.get_frequency()[freq]))
        core.set_speedup(index,speedup)
        print core.get_speedup(index)
...
for core in itertools.combinations(all_configs,int(total_cores)):
    ...
    if(len(allowed_proc_configs) < 200000):
        continue
    else:
        ##allowed_proc_configs=[]
        break
...
elif (total_cores == 2 or (len(allowed_proc_configs) > 0  and  len(allowed_proc_configs) < 200000)):
