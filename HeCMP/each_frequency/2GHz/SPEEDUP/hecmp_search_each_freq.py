...
class core_config():
    def __init__(self):
        ...
        self.app_speedup={}#speedup obtained over base core 477
...
all_configs = [core_config() for i in range (0,649)]
...
for index in xrange(0, len(benchmark_list)):
    for i in xrange (0,648):
        current_benchmark = benchmark_list[index]
        speedup=execution_time(all_configs[648].get_cycles(current_benchmark),all_configs[648].get_frequency())/execution_time(all_configs[i].get_cycles(current_benchmark),all_configs[i].get_frequency())
