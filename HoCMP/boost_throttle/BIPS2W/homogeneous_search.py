...
def evaluate(total_cores, core, area_constraint, power_constraint):
    ...
    bips_permutation_total_value=0.0 ##total core level bips value of the schedule
    bips_schedules=[]
    ...
    for p in itertools.combinations(benchmark_list,total_cores):
        ...
        for f in freq_setting:
            #print f
            bips_permutation_total_value=0.0
            proc_power=0.0
            ...
            if (proc_power > power_constraint):### Drop schedule if greater than power budget
                bips_schedules.append(0)
            else:
                #bips_permutation_total_value = core[0].get_bips2w(benchmark_list[benchmark_list.index(r[0])])[f[0]] + core[1].get_bips2w(benchmark_list[benchmark_list.index(r[1])])[f[1]] + core[2].get_bips2w(benchmark_list[benchmark_list.index(r[2])])[f[2]] + core[3].get_bips2w(benchmark_list[benchmark_list.index(r[3])])[f[3]]
                bips_permutation_total_value = core[0].get_bips2w(wl1)[f[0]] + core[1].get_bips2w(wl2)[f[1]] + core[2].get_bips2w(wl3)[f[2]] + core[3].get_bips2w(wl4)[f[3]]
                bips_schedules.append(bips_permutation_total_value)
        max_bips_combination.append(max(bips_schedules))
...
myFile.write("################ SRI ###############\n")
myFile.write("\n")
myFile.write("Average BIPS^2/W: " + str(bestcore_goodness))
myFile.write("\n"+"\n")

config_detail = optimized_cores[0].get_attribute()
myFile.write( "Core_" + str(optimized_cores[0].get_coreID()) + ";Area:"  + str(optimized_cores[0].get_area()) + ";Peak_Power:" + str(optimized_cores[0].get_core_peak_power()) + ";h264ref_Power:" + str(optimized_cores[0].get_runtime_power(9)) + "\n")
myFile.write("F=" + str(optimized_cores[0].get_frequency()) + "-W=" + config_detail[0] + "-ROB=" + config_detail[1] + "-IQ=" + config_detail[2] + "-LQ=" + config_detail[3] + "-SQ=" + config_detail[4] + "-L1_I$=" + config_detail[5] + "-L1_D$=" + config_detail[6] + "-L2=" + config_detail[7] + "\n" + "\n")
myFile.close()
...
##Frequency
freq=optimized_cores[0].get_frequency()
if (freq==1.0):
    config_detail0=1
elif (freq==1.5):
    config_detail0=2
...
myFile.write(" ,{name: 'core_" + str(optimized_cores[0].get_coreID()) + "'," + "\n")
...
for index in range(len(benchmark_list)):
    myFile.write(str(benchmark_map[index]) + ", config core" + str(optimized_cores[0].get_coreID()) + ", " + str(optimized_cores[0].get_perf(benchmark_list[index])) + ", " + str(optimized_cores[0].get_runtime_power(benchmark_list[index])) + ", " + str(optimized_cores[0].get_bips2w(benchmark_list[index])))
    myFile.write("\n")
myFile.close()
