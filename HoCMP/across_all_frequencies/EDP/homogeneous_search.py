...
def evaluate(total_cores, core, area_constraint, power_constraint):
    ...
    for p in itertools.combinations(benchmark_list,total_cores):
        ...
        for i in total_eval_range:
            current_core=core[i]
            bips_permutation_total_value+=all_configs[current_core].get_edp(p[i])
...
for line in file (sys.argv[1]):
    ...
    all_configs[r].set_edp(bench_map,float(ex_time)*float(ex_time)*(float(runtime_dynamic)+float(total_leakage)))
...
bestcore_edp = evaluate (total_cores ,optimized_cores, area_constraint, power_constraint)
...
for index in range(len(benchmark_list)):
    myFile.write(str(benchmark_map[index]) + ", config core" + str(all_configs[optimized_cores[0]].get_coreID()) + ", " + str(all_configs[optimized_cores[0]].get_perf(benchmark_list[index])) + ", " + str(all_configs[optimized_cores[0]].get_runtime_power(benchmark_list[index])) + ", " + str(all_configs[optimized_cores[0]].get_edp(benchmark_list[index])))
    myFile.write("\n")
myFile.close()
